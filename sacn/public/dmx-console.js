...
class DMXConsole extends HTMLElement {
    constructor() {
        ...
        if (pathParts[0] === 'the-venue-club') pathParts.shift();

        this.deviceName = pathParts.length > 0 ? pathParts[0] : 'default';
        this.isDeepLinked = pathParts.length > 1;
        this.requestedLayout = pathParts.length > 1 ? pathParts[1] : '8';

        this.totalChannels = 512;
        
        if (this.requestedLayout === 'custom') {
            this.physicalFaders = 'custom';
            this.totalBanks = 1; 
        } else if (isNaN(parseInt(this.requestedLayout))) {
            this.physicalFaders = this.requestedLayout; 
            this.totalBanks = 1;
        } else {
            this.physicalFaders = parseInt(this.requestedLayout) || 8;
            this.totalBanks = this.totalChannels / this.physicalFaders;
        }
        
        this.currentBank = 0;
        this.channelState = new Map(); 
        for (let i = 0; i < this.totalChannels; i++) this.channelState.set(`${this.deviceName}:${i}`, this.createEmptyChannel());
        ...
        // Check if PWA is already installed to hide the button on load
        if (navigator.serviceWorker && navigator.serviceWorker.controller) {
            const pwaBtn = this.shadowRoot.getElementById('pwa-btn');
            const removePwaBtn = this.shadowRoot.getElementById('remove-pwa-btn');
            if (pwaBtn) pwaBtn.style.display = 'none';
            if (removePwaBtn) removePwaBtn.style.display = 'inline-block';
        }

        // --- BUG FIX: Force an initial DOM paint so the Simulacrum shows immediately ---
        this.updateVisibleFaders();
    }

    createEmptyChannel() {
        return { value: 0, enabled: true, protected: false, name: '', defaultName: '', glyph: '', radioColor: '', history: [] };
    }

    loadLocalState() {
        try {
            const data = localStorage.getItem(`dmx_state_${this.deviceName}`);
            if (data) {
                const parsed = JSON.parse(data);
                if (parsed.channels) {
                    for (let [key, ch] of Object.entries(parsed.channels)) {
                        if (this.channelState.has(key)) this.channelState.set(key, ch);
                    }
                }
                if (parsed.customLayout) this.customLayout = parsed.customLayout;
                if (parsed.customLayoutName) this.customLayoutName = parsed.customLayoutName;
            }
        } catch (e) {}
    }
    ...
    saveLocalState() {
        ...
        this._saveTimeout = setTimeout(() => {
            try {
                const stateObj = {};
                for (let [key, ch] of this.channelState.entries()) stateObj[key] = ch;
                localStorage.setItem(`dmx_state_${this.deviceName}`, JSON.stringify({ channels: stateObj, customLayout: this.customLayout, customLayoutName: this.customLayoutName }));
            } catch (e) {}
    ...
    getChannel(device, channel) {
        ...
        if (!this.channelState.has(key)) this.channelState.set(key, this.createEmptyChannel());
    ...
    getCustomBanks(layoutArray = this.customLayout) {
        ...
        layoutArray.forEach(item => {
            if (item === 'bank_break') banks.push([]);
            else banks[banks.length - 1].push(item);
        });
        return banks;
    }
    ...
    getExpandedBankChannels(bankItems) {
        ...
        bankItems.forEach(item => {
            if (typeof item === 'string') {
                ...
                } else if (!isNaN(parseInt(item))) expanded.add(`${this.deviceName}:${item}`);
            } else if (typeof item === 'number') expanded.add(`${this.deviceName}:${item}`);
        });
        return expanded;
    }
    ...
    setupExternalListeners() {
        ...
        this.addEventListener('dmx-tunneled', () => {
            const syncBtn = this.shadowRoot.getElementById('sync-btn');
            if (syncBtn) syncBtn.style.display = 'none';
        });

        this.addEventListener('dmx-pwa-installed', () => {
            const pwaBtn = this.shadowRoot.getElementById('pwa-btn');
            const removePwaBtn = this.shadowRoot.getElementById('remove-pwa-btn');
            if (pwaBtn) pwaBtn.style.display = 'none';
            if (removePwaBtn) removePwaBtn.style.display = 'inline-block';
        ...
        this.addEventListener('dmx-pwa-removed', () => {
            const pwaBtn = this.shadowRoot.getElementById('pwa-btn');
            const removePwaBtn = this.shadowRoot.getElementById('remove-pwa-btn');
            if (pwaBtn) pwaBtn.style.display = 'inline-block';
            if (removePwaBtn) removePwaBtn.style.display = 'none';
        });
        ...
        this.addEventListener('dmx-global-meta', (e) => {
            ...
            if (this.physicalFaders === 'custom') { this.renderFadersDOM(); this.updateVisibleFaders(); }
            this.preloadGlyphAssets();
        });

        this.addEventListener('dmx-layout-sync', (e) => {
            this.customLayout = e.detail || [];
            if (this.physicalFaders === 'custom') { this.recalculateTotalBanks(); this.renderFadersDOM(); this.updateVisibleFaders(); }
        });
        ...
        this.addEventListener('dmx-server-layout-sync', (e) => {
            ...
            if (this.serverLayout && this.physicalFaders === this.serverLayout.slug) {
                this.recalculateTotalBanks(); this.renderFadersDOM(); this.updateVisibleFaders();
            }
        });
        ...
        this.addEventListener('dmx-radio-colors', (e) => {
            ...
            colors.forEach((color, i) => {
                ...
                if (chData.radioColor !== color) { chData.radioColor = color; needsUpdate = true; }
            });
        ...
        this.addEventListener('dmx-set', (e) => {
            ...
            if (glyph !== undefined && chData.glyph !== glyph) { chData.glyph = glyph; needsUpdate = true; }
            if (name !== undefined && chData.name !== name) { chData.name = name; needsUpdate = true; }
            if (isProtected !== undefined && chData.protected !== !!isProtected) { chData.protected = !!isProtected; needsUpdate = true; }
            if (enabled !== undefined && chData.enabled !== !!enabled) { chData.enabled = !!enabled; needsUpdate = true; }

            if (value !== undefined) {
                if (source !== 'console' || chData.enabled) {
                    if (chData.value !== value) { chData.value = Math.max(0, Math.min(255, value)); needsUpdate = true; }
                }
            }
            ...
            if (needsUpdate && this.isChannelVisible(device, channel)) this.updateVisibleFaders();
            if (source === 'console' && !chData.enabled && this.isChannelVisible(device, channel)) this.refreshTooltipIfActive(device, channel);
            this.saveLocalState();
        });

        // --- NEW: React to Link Consumption ---
        this.addEventListener('dmx-link-used', (e) => {
            if (e.detail === this.activeToken) {
                this.clearTimers();
                this.activeToken = null;
                
                // Show success screen inside the modal
                this.shadowRoot.getElementById('qr-view').style.display = 'none';
                this.shadowRoot.getElementById('clipboard-view').style.display = 'none';
                this.shadowRoot.getElementById('success-view').style.display = 'flex';
                
                // Auto close
                setTimeout(() => {
                    const modal = this.shadowRoot.getElementById('qr-modal');
                    if (modal.style.display === 'flex') this.toggleModal('qr-modal');
                }, 2000);
            }
        });
    }
    ...
    dispatchChange(device, channel, source) {
        ...
        document.dispatchEvent(new CustomEvent('dmx-change', {
            detail: { device, channel, value: chData.value, enabled: chData.enabled, protected: chData.protected, name: chData.name, source }
        }));
    }

    dispatchPresetAction(action, presetName) {
        document.dispatchEvent(new CustomEvent('dmx-action-preset', { detail: { action: action, name: presetName } }));
    }
    ...
    changeBank(offset) {
        ...
        this.renderFadersDOM(); this.updateVisibleFaders(); this.dispatchBank();
    }

    changeBankSize(newSize) {
        if (newSize === 'custom' || isNaN(parseInt(newSize))) {
            this.physicalFaders = newSize; this.recalculateTotalBanks(); this.renderFadersDOM(); this.updateVisibleFaders(); return;
        }
    ...
    saveLayoutSorter() {
        ...
        this.recalculateTotalBanks(); this.renderFadersDOM(); this.updateVisibleFaders(); this.toggleModal('layout-editor-modal');
    }
    ...
    populateSorterDeviceSelect() {
        ...
        select.innerHTML = html; select.value = this.deviceName; 
    }
    ...
    renderSorterAvailableList() {
        ...
        if (!meta || !meta.names) {
            listContainer.innerHTML = '<div style="color: #6c7086; padding: 10px;">No channels found.</div>'; return;
        }

        const panelName = meta.serverLayoutName ? meta.serverLayoutName : selectedDevice;
        const radioChannels = new Set();
        if (meta.radioGroups) meta.radioGroups.forEach(g => g.forEach(ch => radioChannels.add(ch)));

        let html = '';
        if (meta.radioGroups) {
            meta.radioGroups.forEach((group, gIdx) => {
                const groupNames = group.map(ch => meta.names[ch] || meta.defaultNames[ch] || `CH ${ch+1}`).join(' / ');
                html += `<div class="sorter-item" style="border-left: 3px solid #f9e2af;"><span><strong>${panelName}</strong><br><span style="font-size:0.75em; color:#a6adc8;">${groupNames}</span></span><button class="preset-btn load" data-action="add-group" data-device="${selectedDevice}" data-group="${gIdx}">+</button></div>`;
            });
        }
        for (let i = 0; i < 512; i++) {
            ...
            html += `<div class="sorter-item"><span>${displayLabel}</span><button class="preset-btn load" data-action="add-channel" data-device="${selectedDevice}" data-channel="${i}">+</button></div>`;
        }
    ...
    renderSorterSelectedList() {
        ...
        if (this.tempLayout.length === 0) { listContainer.innerHTML = '<div style="color: #6c7086; text-align: center; margin-top: 20px;">Layout is empty.</div>'; return; }

        let html = '';
        this.tempLayout.forEach((item, index) => {
            if (item === 'bank_break') {
                html += `<div class="sorter-item selected-item" style="background: #45475a; justify-content: center; position: relative;"><strong style="color: #a6e3a1; letter-spacing: 2px;">--- PAGE BREAK ---</strong><div class="sorter-actions" style="position: absolute; right: 12px;"><button class="preset-btn" data-action="move-up" data-index="${index}" ${index === 0 ? 'disabled' : ''}>▲</button><button class="preset-btn" data-action="move-down" data-index="${index}" ${index === this.tempLayout.length - 1 ? 'disabled' : ''}>▼</button><button class="preset-btn delete" data-action="remove-channel" data-index="${index}">X</button></div></div>`;
                return; 
            }
            if (typeof item === 'string' && item.includes(':group:')) {
                const parts = item.split(':'); const dev = parts[0]; const gIdx = parseInt(parts[2]);
                let panelName = dev;
                if (this.globalMeta[dev] && this.globalMeta[dev].serverLayoutName) panelName = this.globalMeta[dev].serverLayoutName;
                let groupLabel = "Unknown Group";
                if (this.globalMeta[dev] && this.globalMeta[dev].radioGroups && this.globalMeta[dev].radioGroups[gIdx]) {
                    groupLabel = this.globalMeta[dev].radioGroups[gIdx].map(ch => this.globalMeta[dev].names[ch] || this.globalMeta[dev].defaultNames[ch] || `CH ${ch+1}`).join(' / ');
                }
                ...
                html += `<div class="sorter-item selected-item" style="border-color: #f9e2af;"><span style="display:flex; align-items:flex-start; gap: 8px;"><span style="color:#6c7086; font-size:0.8em; width:20px; padding-top:2px;">${index+1}.</span><span>${displayLabel}</span></span><div class="sorter-actions"><button class="preset-btn" data-action="move-up" data-index="${index}" ${index === 0 ? 'disabled' : ''}>▲</button><button class="preset-btn" data-action="move-down" data-index="${index}" ${index === this.tempLayout.length - 1 ? 'disabled' : ''}>▼</button><button class="preset-btn delete" data-action="remove-channel" data-index="${index}">X</button></div></div>`;
                return; 
            }

            let dev = this.deviceName; let ch = item;
            if (typeof item === 'string' && item.includes(':')) { const parts = item.split(':'); dev = parts[0]; ch = parseInt(parts[1]); } 
            else if (typeof item === 'string') { ch = parseInt(item); }

            let chName = '';
            if (this.globalMeta[dev] && this.globalMeta[dev].names[ch]) chName = this.globalMeta[dev].names[ch];
            else if (this.globalMeta[dev] && this.globalMeta[dev].defaultNames[ch]) chName = this.globalMeta[dev].defaultNames[ch];
            else { const stateData = this.getChannel(dev, ch); chName = stateData.name || stateData.defaultName || ''; }
            ...
            html += `<div class="sorter-item selected-item"><span style="display:flex; align-items:center; gap: 8px;"><span style="color:#6c7086; font-size:0.8em; width:20px;">${index+1}.</span>${displayLabel}</span><div class="sorter-actions"><button class="preset-btn" data-action="move-up" data-index="${index}" ${index === 0 ? 'disabled' : ''}>▲</button><button class="preset-btn" data-action="move-down" data-index="${index}" ${index === this.tempLayout.length - 1 ? 'disabled' : ''}>▼</button><button class="preset-btn delete" data-action="remove-channel" data-index="${index}">X</button></div></div>`;
        });
        listContainer.innerHTML = html;
    }

    openNameEditor(device, channel) {
        this.editingDevice = device; this.editingChannel = channel;
        const chData = this.getChannel(device, channel);
        if (this.globalMeta[device] && this.globalMeta[device].defaultNames) chData.defaultName = this.globalMeta[device].defaultNames[channel] || '';
        ...
        if (chData.defaultName) { restoreBtn.style.opacity = '1'; restoreBtn.style.pointerEvents = 'auto'; } 
        else { restoreBtn.style.opacity = '0.3'; restoreBtn.style.pointerEvents = 'none'; }
    ...
    closeNameEditor() {
        ...
        this.editingDevice = null; this.editingChannel = null;
    }
    ...
    saveEditedName() {
        if (this.editingDevice !== null && this.editingChannel !== null) {
            ...
            if (this.globalMeta[this.editingDevice]) {
                if (!this.globalMeta[this.editingDevice].names) this.globalMeta[this.editingDevice].names = new Array(512).fill('');
            ...
            if (layoutModal && layoutModal.style.display === 'flex') { this.renderSorterAvailableList(); this.renderSorterSelectedList(); }
            this.dispatchChange(this.editingDevice, this.editingChannel, 'human');
            this.closeNameEditor();
        }
    }

    // --- NEW: Robust Clipboard Polyfill ---
    copyToClipboard(url) {
        const successCb = () => {};
        const failCb = (err) => console.error('Failed to copy text: ', err);

        if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(url).then(successCb).catch(failCb);
        } else {
            ...
            textArea.focus();
            textArea.select();
            try { document.execCommand('copy'); } catch (err) { failCb(err); }
            document.body.removeChild(textArea);
        }
    }

    // --- NEW: Shortlink Generator & Timer Logic ---
    clearTimers() {
        if (this.linkTimer) clearInterval(this.linkTimer);
    }

    startCountdown(type, ttl, expiresAt, url) {
        this.clearTimers();
        
        if (type === 'clipboard') {
            this.shadowRoot.getElementById('qr-view').style.display = 'none';
            this.shadowRoot.getElementById('clipboard-view').style.display = 'flex';
            this.shadowRoot.getElementById('revoke-url-btn').style.display = 'block';
            this.copyToClipboard(url);
        } else {
            this.shadowRoot.getElementById('qr-view').style.display = 'flex';
            this.shadowRoot.getElementById('clipboard-view').style.display = 'none';
            const qrContainer = this.shadowRoot.getElementById('qrcode');
            qrContainer.innerHTML = '';
            new QRCode(qrContainer, { text: url, width: 220, height: 220, colorDark : "#000000", colorLight : "#ffffff", correctLevel : QRCode.CorrectLevel.H });
        }

        const fillEl = this.shadowRoot.getElementById(type === 'qr' ? 'qr-progress-fill' : 'clip-progress-fill');
        const textEl = this.shadowRoot.getElementById(type === 'qr' ? 'qr-time-text' : 'clip-time-text');
        ...
        this.linkTimer = setInterval(() => {
            ...
            if (left <= 0) {
                this.clearTimers();
                if (type === 'qr') {
                    this.generateLink('qr'); // Auto-refresh QR
                } else {
                    fillEl.style.width = '0%';
                    textEl.textContent = 'Link Expired';
                    this.shadowRoot.getElementById('revoke-url-btn').style.display = 'none';
                }
                return;
            }
            
            fillEl.style.width = `${(left / ttl) * 100}%`;
            
            const secs = Math.ceil(left / 1000);
            if (type === 'clipboard') {
                const m = Math.floor(secs / 60);
                const s = (secs % 60).toString().padStart(2, '0');
                textEl.textContent = `Valid for ${m}:${s}`;
            } else {
                textEl.textContent = `Expires in ${secs}s`;
            }
        }, 250); 
    }

    async generateLink(type) {
        const target = `${this.deviceName}/${this.physicalFaders}`;
        let pathParts = window.location.pathname.split('/').filter(Boolean);
        let rootPath = pathParts[0] === 'the-venue-club' ? '/' + pathParts.shift() : "";
        
        try {
            const response = await fetch(`${rootPath}/api/gateway/link?target=${encodeURIComponent(target)}&type=${type}&device=${this.deviceName}`);
            const data = await response.json();
            this.activeToken = data.token;
            this.startCountdown(type, data.ttl, data.expires, data.url);
        } catch (err) {
    ...
    toggleModal(modalId) {
        ...
        if (modalId === 'qr-modal') {
            if (!isVisible) {
                ...
                this.shadowRoot.getElementById('qr-view').style.display = 'flex';
                this.shadowRoot.getElementById('clipboard-view').style.display = 'none';
                this.shadowRoot.getElementById('success-view').style.display = 'none';
                this.shadowRoot.getElementById('qrcode').innerHTML = '<div style="color:#6c7086; padding: 20px; font-family: sans-serif;">Securing gateway link...</div>';
                this.generateLink('qr');
            } else {
                // If closing modal, explicitly revoke the active physical token for security
                this.clearTimers();
                if (this.activeToken) {
                    document.dispatchEvent(new CustomEvent('dmx-revoke-link', { detail: this.activeToken }));
                    this.activeToken = null;
                }
            }
        }
    ...
    populateTooltip(device, channel, tooltipElement) {
        ...
        if (!chData.history || chData.history.length === 0) {
            tooltipElement.innerHTML = `<strong>CH ${displayChannel}${devLabel}${channelName} Blocked</strong><br>No recent console activity.`; return;
        }
        ...
        chData.history.forEach(entry => {
            ...
            html += `<div style="display:flex; justify-content:space-between; width: 120px;"><span>${timeStr}</span> <span style="color:#a6e3a1;">Val: ${entry.value}</span></div>`;
        });
        html += `</div>`;
    ...
    updateVisibleFaders() {
        ...
        if (this.serverLayout && this.physicalFaders === this.serverLayout.slug) {
            prevBankBtn.style.visibility = 'hidden'; nextBankBtn.style.visibility = 'hidden';
            bankLabel.textContent = this.serverLayout.name; editLayoutBtn.style.display = 'none';
        } else if (this.physicalFaders === 'custom') {
            if (this.totalBanks > 1) {
                prevBankBtn.style.visibility = 'visible'; nextBankBtn.style.visibility = 'visible';
            ...
            if (this.totalBanks > 1) { prevBankBtn.style.visibility = 'visible'; nextBankBtn.style.visibility = 'visible'; } 
            else { prevBankBtn.style.visibility = 'hidden'; nextBankBtn.style.visibility = 'hidden'; }
        ...
        const strips = this.shadowRoot.querySelectorAll('.fader-strip');
        strips.forEach((strip) => {
            const device = strip.dataset.device; const channelIndex = parseInt(strip.dataset.channel);
            ...
            if (chData.radioColor) strip.classList.add(`radio-${chData.radioColor}`);
            strip.querySelector('.channel-label').textContent = channelIndex + 1;

            const nameBadge = strip.querySelector('.channel-name');
            nameBadge.textContent = chData.name || '---'; nameBadge.className = `channel-name ${chData.name ? '' : 'empty'}`;

            const glyphBadge = strip.querySelector('.channel-glyph');
            if (chData.glyph) {
                glyphBadge.classList.add('has-content');
                if (chData.glyph.trim().startsWith('<')) { glyphBadge.style.backgroundImage = 'none'; glyphBadge.innerHTML = chData.glyph; } 
                else { glyphBadge.style.backgroundImage = `url('${chData.glyph}')`; glyphBadge.innerHTML = ''; }
            } else { glyphBadge.style.backgroundImage = 'none'; glyphBadge.innerHTML = ''; glyphBadge.classList.remove('has-content'); }
            ...
            if (chData.protected) {
                strip.classList.add('is-protected'); slider.classList.add('radio-locked'); slider.style.opacity = chData.enabled ? '0.8' : '0.4'; 
                btn.textContent = chData.enabled ? '🔒 CONSOLE' : '🔒 MANUAL'; btn.className = `toggle-btn protected-btn ${chData.enabled ? 'active' : ''}`;
            } else {
                strip.classList.remove('is-protected');
                if (isRadioCh) slider.classList.add('radio-locked'); else slider.classList.remove('radio-locked');
                slider.style.opacity = chData.enabled ? '1' : '0.5'; 
                btn.textContent = chData.enabled ? 'CONSOLE' : 'MANUAL'; btn.className = `toggle-btn ${chData.enabled ? 'active' : ''}`;
                btn.className = `toggle-btn ${chData.enabled ? 'active' : ''}`;
            }

            // --- NEW: Toggle slider color based on state ---
            if (chData.enabled) {
                slider.classList.remove('state-manual');
            } else {
                slider.classList.add('state-manual');
            }
    ...
    renderPresetListDOM() {
        ...
        if (!listContainer) return;
        if (this.presetList.length === 0) { listContainer.innerHTML = '<div style="color: #6c7086; text-align: center;">No presets saved yet.</div>'; return; }
        let html = '';
        this.presetList.forEach(presetName => {
            html += `<div class="preset-item"><span class="preset-name">${presetName}</span><div class="preset-actions"><button class="preset-btn load" data-action="load" data-name="${presetName}">Load</button><button class="preset-btn delete" data-action="delete" data-name="${presetName}">X</button></div></div>`;
        });
        listContainer.innerHTML = html;
    }
    ...
    renderFadersDOM() {
        ...
        if (this.physicalFaders === 'custom') {
            ...
            currentBankItems.forEach(item => {
                if (typeof item === 'number') { activeFaders.push({ device: this.deviceName, channel: item }); } 
                else if (typeof item === 'string') {
                    if (item.includes(':group:')) {
                        const parts = item.split(':'); const dev = parts[0]; const gIdx = parseInt(parts[2]);
                        if (this.globalMeta[dev] && this.globalMeta[dev].radioGroups && this.globalMeta[dev].radioGroups[gIdx]) {
                            this.globalMeta[dev].radioGroups[gIdx].forEach(ch => { activeFaders.push({ device: dev, channel: ch }); });
                        }
                    } else if (item.includes(':')) {
                        const [dev, target] = item.split(':'); if (!isNaN(parseInt(target))) activeFaders.push({ device: dev, channel: parseInt(target) });
                    } else if (!isNaN(parseInt(item))) activeFaders.push({ device: this.deviceName, channel: parseInt(item) });
                }
            });
        } else if (this.serverLayout && this.physicalFaders === this.serverLayout.slug) {
            this.serverLayout.channels.forEach(ch => { activeFaders.push({ device: this.deviceName, channel: parseInt(ch) }); });
        } else {
            const startChannel = this.currentBank * parseInt(this.physicalFaders);
            for (let i = 0; i < parseInt(this.physicalFaders); i++) activeFaders.push({ device: this.deviceName, channel: startChannel + i });
        }

        let fadersHtml = '';
        activeFaders.forEach(fader => {
            fadersHtml += `<div class="fader-strip" data-device="${fader.device}" data-channel="${fader.channel}"><div class="led-green"></div><div class="led-red"></div><input type="range" min="0" max="255" value="0" class="fader-input" data-device="${fader.device}" data-channel="${fader.channel}"><button class="toggle-btn" data-device="${fader.device}" data-channel="${fader.channel}"></button><div class="channel-label">${fader.channel + 1}</div><div class="channel-name empty" data-device="${fader.device}" data-channel="${fader.channel}">---</div><div class="channel-glyph" data-device="${fader.device}" data-channel="${fader.channel}"></div></div>`;
        });
    ...
    updateSelectOptions() {
        ...
        if (!select) return;
        let html = `<option value="4" ${this.physicalFaders === 4 ? 'selected' : ''}>4 CH</option><option value="8" ${this.physicalFaders === 8 ? 'selected' : ''}>8 CH</option><option value="16" ${this.physicalFaders === 16 ? 'selected' : ''}>16 CH</option><option value="32" ${this.physicalFaders === 32 ? 'selected' : ''}>32 CH</option><option value="64" ${this.physicalFaders === 64 ? 'selected' : ''}>64 CH</option><option value="custom" ${this.physicalFaders === 'custom' ? 'selected' : ''}>Custom</option>`;
        if (this.serverLayout) html += `<option value="${this.serverLayout.slug}" ${this.physicalFaders === this.serverLayout.slug ? 'selected' : ''}>${this.serverLayout.name}</option>`;
        select.innerHTML = html;
    }

    // --- NEW: Aggressive Asset Preloader ---
    preloadGlyphAssets() {
        const urlsToCache = new Set();

        // 1. Scan global metadata for any remote glyph URLs
        for (const dev in this.globalMeta) {
            const glyphs = this.globalMeta[dev].glyphs;
            if (glyphs) {
                glyphs.forEach(g => {
                    if (g && !g.trim().startsWith('<')) { // If it doesn't start with <svg, assume it's a URL
                        urlsToCache.add(g.trim());
                    }
                });
            }
        }

        // 2. Scan the local channel state just in case
        this.channelState.forEach(ch => {
            if (ch.glyph && !ch.glyph.trim().startsWith('<')) urlsToCache.add(ch.glyph.trim());
        });

        // 3. Silently fetch them. The Service Worker will intercept these and cache them permanently for offline use.
        urlsToCache.forEach(url => {
            fetch(url, { mode: 'no-cors' }).catch(() => {
                // Silently fail if offline, the SW will serve from cache later anyway
            });
        });
    }
    ...
    render() {
        ...
        const style = `
            ...
            .sync-btn.purple { background: #cba6f7; color: #11111b; }
            .sync-btn.purple:hover { background: #f5c2e7; }
            .sync-btn.delete { background: #f38ba8; color: #11111b; }
            .sync-btn.delete:hover { background: #eba0ac; }
            ...
            .fader-input { 
                ...
                margin: 15px 0; 
                cursor: pointer; 
                transition: opacity 0.2s, accent-color 0.2s; 
                accent-color: #a6e3a1; /* CONSOLE mode: Green */
            }
            .fader-input.state-manual { 
                accent-color: #f38ba8; /* MANUAL mode: Red */
            }
            ...
            .modal-header { display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #45475a; padding-bottom: 10px; margin-bottom: 15px; margin-top: 0; width: 100%; }
            ...
            .offline-modal { background-color: rgba(17, 17, 27, 0.3); backdrop-filter: blur(6px) grayscale(70%); -webkit-backdrop-filter: blur(6px) grayscale(70%); z-index: 200; display: flex; flex-direction: column; }
            .offline-spinner { width: 50px; height: 50px; border: 5px solid rgba(255,255,255,0.1); border-top: 5px solid #f38ba8; border-radius: 50%; animation: spin 1s linear infinite; }
            ...
            .progress-bar { width: 100%; height: 6px; background: #313244; margin-top: 15px; border-radius: 3px; overflow: hidden; }
            .progress-fill { height: 100%; background: #89dceb; width: 100%; transition: width 0.25s linear; }
        `;
        ...
        this.shadowRoot.innerHTML = `
            ...
            <div class="controls">
                ...
                <button class="sync-btn" id="menu-btn" style="background: #313244; color: #cdd6f4; font-size: 1.2em; padding: 4px 12px; margin-left: 10px;" style="${hideUiStyle}">&vellip;</button>
            </div>
            <div class="fader-bank"></div>
            
            <div class="modal-overlay" id="menu-modal">
                <div class="modal-content" style="width: 250px; align-items: center; border: 1px solid #45475a;">
                    <h2 class="modal-header">
                        Menu
                        <button class="modal-close" id="close-menu">&times;</button>
                    </h2>
                    <div style="display: flex; flex-direction: column; gap: 15px; width: 100%; margin-top: 10px;">
                        <button class="sync-btn blue" id="presets-btn" style="padding: 12px; font-size: 1em;"> Presets </button>
                        <button class="sync-btn orange" id="sync-btn" style="padding: 12px; font-size: 1em;"> Other Devices </button>
                        <button class="sync-btn purple" id="pwa-btn" style="padding: 12px; font-size: 1em;"> Install App </button>
                        <button class="sync-btn delete" id="remove-pwa-btn" style="padding: 12px; font-size: 1em; display: none;"> Remove App </button>
                    </div>
                </div>
            </div>

            <div class="modal-overlay" id="qr-modal">
                <div class="modal-content" style="border: 1px solid #45475a; min-width: auto; max-width: 320px; display: flex; flex-direction: column; align-items: center;">
                    <h2 class="modal-header">
                        Sync Device
                        <button class="modal-close" id="close-qr">&times;</button>
                    </h2>
                    
                    <div id="qr-view" style="width: 100%; display: flex; flex-direction: column; align-items: center;">
                        <div id="qrcode" style="background: white; padding: 10px; border-radius: 8px;"></div>
                        <div class="progress-bar"><div id="qr-progress-fill" class="progress-fill"></div></div>
                        <div id="qr-time-text" style="font-size:0.85em; color:#a6adc8; margin-top:5px;">Expires in --s</div>
                        
                        <div style="margin-top: 15px; width: 100%; border-top: 1px solid #45475a; padding-top: 15px;">
                            <button id="copy-url-btn" class="preset-btn" style="width: 100%; padding: 10px; background: #89b482; color: #11111b; font-size: 1em;">Email / Copy Link</button>
                        </div>
                    </div>

                    <div id="clipboard-view" style="display:none; width: 100%; flex-direction: column; align-items: center; text-align: center; padding: 20px 0;">
                        <h3 style="color:#a6e3a1; margin:0 0 10px 0;">Link Copied!</h3>
                        <p style="font-size:0.9em; color:#cdd6f4; line-height: 1.4; margin: 0;">The secure shortlink has been copied to your clipboard.</p>
                        
                        <div class="progress-bar" style="margin-top: 15px;"><div id="clip-progress-fill" class="progress-fill" style="background:#f9e2af;"></div></div>
                        <div id="clip-time-text" style="font-size:0.85em; color:#a6adc8; margin-top:5px; margin-bottom: 20px;">Valid for 5:00</div>
                        
                        <button id="revoke-url-btn" class="preset-btn delete" style="width:100%; padding:10px; font-size: 1em;">Revoke Link Now</button>
                    </div>

                    <div id="success-view" style="display:none; width: 100%; flex-direction: column; align-items: center; text-align: center; padding: 30px 0;">
                        <div style="width: 60px; height: 60px; border-radius: 50%; background: #a6e3a1; color: #11111b; display: flex; align-items: center; justify-content: center; font-size: 30px; font-weight: bold; margin-bottom: 15px;">✓</div>
                        <h3 style="color:#a6e3a1; margin:0;">Device Connected</h3>
                    </div>
                </div>
            </div>
        ...
        // --- Unified Menu Button Event Listeners ---
        const menuBtn = this.shadowRoot.getElementById('menu-btn');
        if (menuBtn) {
            menuBtn.addEventListener('click', () => {
                this.toggleModal('menu-modal');
            });
        }
        
        this.shadowRoot.getElementById('close-menu').addEventListener('click', () => this.toggleModal('menu-modal'));
        this.shadowRoot.getElementById('menu-modal').addEventListener('click', (e) => {
            if (e.target.id === 'menu-modal') this.toggleModal('menu-modal');
        });

        // --- Menu Item Listeners (Route to other modals) ---
        const pwaBtn = this.shadowRoot.getElementById('pwa-btn');
        if (pwaBtn) {
            pwaBtn.addEventListener('click', () => {
                this.toggleModal('menu-modal'); // Close menu first
                document.dispatchEvent(new CustomEvent('dmx-install-pwa'));
            });
        }

        const removePwaBtn = this.shadowRoot.getElementById('remove-pwa-btn');
        if (removePwaBtn) {
            removePwaBtn.addEventListener('click', () => {
                this.toggleModal('menu-modal'); // Close menu first
                if(confirm("Are you sure you want to remove offline support?")) {
                    document.dispatchEvent(new CustomEvent('dmx-remove-pwa'));
                }
            });
        }

        this.shadowRoot.getElementById('sync-btn').addEventListener('click', () => {
            this.toggleModal('menu-modal'); // Close menu first
            this.toggleModal('qr-modal');
        });

        this.shadowRoot.getElementById('presets-btn').addEventListener('click', () => {
            this.toggleModal('menu-modal'); // Close menu first
            this.toggleModal('presets-modal');
        });

        // --- Modal Background Clicks ---
        this.shadowRoot.getElementById('qr-modal').addEventListener('click', (e) => {
            if (e.target.id === 'qr-modal') this.toggleModal('qr-modal');
        });
        
        this.shadowRoot.getElementById('close-qr').addEventListener('click', () => {
            this.toggleModal('qr-modal');
        });
        ...
        this.shadowRoot.getElementById('add-bank-break-btn').addEventListener('click', () => {
            this.tempLayout.push('bank_break'); this.renderSorterSelectedList();
            const list = this.shadowRoot.getElementById('editor-selected-list'); list.scrollTop = list.scrollHeight;
        });
        ...
        this.shadowRoot.getElementById('editor-available-list').addEventListener('click', (e) => {
            ...
            if (editableName) return this.openNameEditor(editableName.dataset.device, parseInt(editableName.dataset.channel));

            if (e.target.dataset.action === 'add-channel') {
                const dev = e.target.dataset.device; const ch = parseInt(e.target.dataset.channel);
                const banks = this.getCustomBanks(this.tempLayout); const currentBank = banks[banks.length - 1]; 
                if (this.getExpandedBankChannels(currentBank).has(`${dev}:${ch}`)) return alert("This channel is already in the current layout page/bank.");
                this.tempLayout.push(`${dev}:${ch}`); this.renderSorterSelectedList();
                const list = this.shadowRoot.getElementById('editor-selected-list'); list.scrollTop = list.scrollHeight;
            }
            if (e.target.dataset.action === 'add-group') {
                const dev = e.target.dataset.device; const gIdx = parseInt(e.target.dataset.group);
                const banks = this.getCustomBanks(this.tempLayout); const currentBank = banks[banks.length - 1]; 
                const expandedCurrentBank = this.getExpandedBankChannels(currentBank);
                let conflict = false;
                if (this.globalMeta[dev] && this.globalMeta[dev].radioGroups && this.globalMeta[dev].radioGroups[gIdx]) {
                    for (const ch of this.globalMeta[dev].radioGroups[gIdx]) if (expandedCurrentBank.has(`${dev}:${ch}`)) conflict = true;
                }
                if (conflict) return alert("One or more channels from this group are already in the current layout page/bank.");
                this.tempLayout.push(`${dev}:group:${gIdx}`); this.renderSorterSelectedList();
                const list = this.shadowRoot.getElementById('editor-selected-list'); list.scrollTop = list.scrollHeight;
            }
        });
        ...
        this.shadowRoot.getElementById('editor-selected-list').addEventListener('click', (e) => {
            const editableName = e.target.closest('.editable-sorter-name');
            if (editableName) return this.openNameEditor(editableName.dataset.device, parseInt(editableName.dataset.channel));
            const action = e.target.dataset.action; const index = parseInt(e.target.dataset.index);
            if (action === 'remove-channel') { this.tempLayout.splice(index, 1); this.renderSorterSelectedList(); } 
            else if (action === 'move-up' && index > 0) { [this.tempLayout[index - 1], this.tempLayout[index]] = [this.tempLayout[index], this.tempLayout[index - 1]]; this.renderSorterSelectedList(); } 
            else if (action === 'move-down' && index < this.tempLayout.length - 1) { [this.tempLayout[index + 1], this.tempLayout[index]] = [this.tempLayout[index], this.tempLayout[index + 1]]; this.renderSorterSelectedList(); }
        });
        
        this.shadowRoot.getElementById('copy-url-btn').addEventListener('click', () => {
            this.generateLink('clipboard');
        });

        this.shadowRoot.getElementById('revoke-url-btn').addEventListener('click', () => {
            if (this.activeToken) {
                document.dispatchEvent(new CustomEvent('dmx-revoke-link', { detail: this.activeToken }));
                this.activeToken = null;
            }
            this.clearTimers();
            this.shadowRoot.getElementById('revoke-url-btn').style.display = 'none';
            this.shadowRoot.getElementById('clip-time-text').textContent = 'Link Revoked Manually';
            this.shadowRoot.getElementById('clip-progress-fill').style.width = '0%';
        });

        this.shadowRoot.getElementById('size-select').addEventListener('change', (e) => this.changeBankSize(e.target.value));

        this.shadowRoot.getElementById('save-preset-btn').addEventListener('click', () => {
            const input = this.shadowRoot.getElementById('new-preset-name'); const name = input.value.trim();
            if (name) { this.dispatchPresetAction('save-preset', name); input.value = ''; }
        });

        this.shadowRoot.getElementById('preset-list-container').addEventListener('click', (e) => {
            if (e.target.classList.contains('preset-btn')) {
                const action = e.target.dataset.action; const presetName = e.target.dataset.name;
                if (action === 'delete') { if (!confirm(`Delete preset '${presetName}'?`)) return; }
        ...
        this.shadowRoot.getElementById('name-edit-modal').addEventListener('click', (e) => { if (e.target.id === 'name-edit-modal') this.closeNameEditor(); });
        this.shadowRoot.getElementById('name-edit-input').addEventListener('keydown', (e) => { if (e.key === 'Enter') this.saveEditedName(); });
        this.shadowRoot.getElementById('save-name-btn').addEventListener('click', () => this.saveEditedName());
        this.shadowRoot.getElementById('clear-name-btn').addEventListener('click', () => { this.shadowRoot.getElementById('name-edit-input').value = ''; this.shadowRoot.getElementById('name-edit-input').focus(); });
        
        this.shadowRoot.getElementById('restore-name-btn').addEventListener('click', () => {
            if (this.editingDevice !== null && this.editingChannel !== null) {
                const chData = this.getChannel(this.editingDevice, this.editingChannel);
                if (chData.defaultName) { this.shadowRoot.getElementById('name-edit-input').value = chData.defaultName; this.shadowRoot.getElementById('name-edit-input').focus(); }
            }
        });
        ...
        faderBank.addEventListener('mouseover', (e) => {
            ...
            if (strip) {
                const device = strip.dataset.device; const channel = parseInt(strip.dataset.channel); const chData = this.getChannel(device, channel);
                const tooltip = this.shadowRoot.getElementById('history-tooltip');
                if (!chData.enabled) {
                    this.populateTooltip(device, channel, tooltip); strip.appendChild(tooltip); 
                    tooltip.style.display = 'block'; tooltip.dataset.device = device; tooltip.dataset.channel = channel;
                }
            }
        });
        ...
        faderBank.addEventListener('mouseout', (e) => {
            const strip = e.target.closest('.fader-strip');
            if (strip) { const tooltip = this.shadowRoot.getElementById('history-tooltip'); if (tooltip) tooltip.style.display = 'none'; }
        });

        faderBank.addEventListener('input', (e) => {
            if (e.target.classList.contains('fader-input')) {
                const strip = e.target.closest('.fader-strip'); const device = strip.dataset.device; const channel = parseInt(strip.dataset.channel); const chData = this.getChannel(device, channel);
                if (chData.protected) return; 
                chData.value = parseInt(e.target.value); chData.enabled = false; this.updateVisibleFaders();
                const tooltip = this.shadowRoot.getElementById('history-tooltip'); if (tooltip) this.populateTooltip(device, channel, tooltip);
                this.dispatchChange(device, channel, 'human');
            }   
        });

        faderBank.addEventListener('click', (e) => {
            const strip = e.target.closest('.fader-strip'); if (!strip) return;
            const device = strip.dataset.device; const channel = parseInt(strip.dataset.channel); const chData = this.getChannel(device, channel);
            if (e.target.classList.contains('channel-name')) return this.openNameEditor(device, channel);
            ...
            if (e.target.classList.contains('toggle-btn')) {
                if (e.shiftKey) { chData.protected = !chData.protected; this.updateVisibleFaders(); this.dispatchChange(device, channel, 'human'); return; }
                if (chData.protected) return; 
                let targetGroup = null;
                if (this.globalMeta[device] && this.globalMeta[device].radioGroups) targetGroup = this.globalMeta[device].radioGroups.find(g => g.includes(channel));
                const newEnabledState = !chData.enabled;
                if (targetGroup) {
                    targetGroup.forEach(ch => {
                        const groupChData = this.getChannel(device, ch); if (groupChData.protected) return;
                        groupChData.enabled = newEnabledState;
                        if (groupChData.enabled && groupChData.history && groupChData.history.length > 0) groupChData.value = groupChData.history[0].value;
                        this.dispatchChange(device, ch, 'human');
                    });
                } else {
                    chData.enabled = newEnabledState;
                    if (chData.enabled && chData.history && chData.history.length > 0) chData.value = chData.history[0].value;
                    this.dispatchChange(device, channel, 'human');
                }
                if (newEnabledState) { const tooltip = this.shadowRoot.getElementById('history-tooltip'); if (tooltip) tooltip.style.display = 'none'; }
                this.updateVisibleFaders();
            }
        });
    }    
