...
function connectWebSocket() {
    ...
    ws = new WebSocket(`${wsProtocol}//${window.location.host}/ui/${deviceName}`);
    ...
    ws.onmessage = (event) => {
        ...
        if (data.type === 'init') {
            ...
            // NEW: Ingest global metadata immediately on boot
            if (data.globalMeta) {
                consoleComponent.dispatchEvent(new CustomEvent('dmx-global-meta', { detail: data.globalMeta }));
            }
            
            if (data.customLayout) {
                consoleComponent.dispatchEvent(new CustomEvent('dmx-layout-sync', { detail: data.customLayout }));
            }
            if (data.customLayoutName) {
                consoleComponent.dispatchEvent(new CustomEvent('dmx-layout-name-sync', { detail: data.customLayoutName }));
            }
            if (data.serverLayout) {
                consoleComponent.dispatchEvent(new CustomEvent('dmx-server-layout-sync', { detail: data.serverLayout }));
            }
            if (data.radioColors) {
                consoleComponent.dispatchEvent(new CustomEvent('dmx-radio-colors', { detail: data.radioColors }));
            }
            ...
            for (let i = 0; i < 512; i++) {
                consoleComponent.dispatchEvent(new CustomEvent('dmx-set', {
                    detail: { 
                        ...
                        protected: data.protected[i],
                        name: data.names[i],
                        defaultName: data.defaultNames ? data.defaultNames[i] : '', 
                        glyph: data.glyphs ? data.glyphs[i] : '', 
...
document.addEventListener('dmx-request-meta', () => {
    if (ws && ws.readyState === WebSocket.OPEN) {
        ws.send(JSON.stringify({ type: 'get-global-meta' }));
    }
});

document.addEventListener('dmx-action-preset', (e) => {
    if (ws && ws.readyState === WebSocket.OPEN) {
        ws.send(JSON.stringify({ type: e.detail.action, name: e.detail.name }));
    }
});

document.addEventListener('dmx-layout-change', (e) => {
    if (ws && ws.readyState === WebSocket.OPEN) {
        ws.send(JSON.stringify({ type: 'update-layout', layout: e.detail }));
    }
});

document.addEventListener('dmx-layout-name-change', (e) => {
    if (ws && ws.readyState === WebSocket.OPEN) {
        ws.send(JSON.stringify({ type: 'update-layout-name', name: e.detail }));
    }
});